...
export function SessionEditor({
  ...
  function finishSelect(e: React.MouseEvent) {
    ...
    if (hi > lo && !locked && !hasOptimisticRows) {
        ...
        <div className="flex flex-col gap-2 select-none" onMouseUp={finishSelect}>
          ...
          {blocks.map((block) => {
            if (block.kind === "single") {
              ...
              const isOptimistic = (row as any)._optimistic === true;
              return (
                <RowLine
                  key={row.id}
                  row={row}
                    ...
                    {block.rows.map((row, k) => {
                      ...
                      return (
                        <RowLine
                          ...
                          selected={isRowInSelection(index)}
                          locked={locked}
                          optimistic={(row as any)._optimistic === true}
                          onMouseDown={(e) => startSelect(e, index)}
                          onMouseEnter={() => extendSelect(index)}
                          onDragOver={(e) => e.preventDefault()}
                          onDrop={() => handleDrop(index)}
...
function RowLine({
  ...
  return (
    <div
      ...
      style={{ borderLeft: row.groupColor ? `4px solid ${row.groupColor}` : undefined, position: "relative", paddingBottom: row.clientNote ? "24px" : undefined }}
    ...
    >
      ...
      {/* Client note — always shown when present */}
      {row.clientNote && (
