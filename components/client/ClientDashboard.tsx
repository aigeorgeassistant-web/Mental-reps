...
function ProgressGraph({ sets, view, lowerIsBetter }: { sets: LoggedSet[]; view: GraphView; lowerIsBetter: boolean }) {
  const points = useMemo(() => {
    ...
    return [...bySession.entries()]
      .map(([, sessionSets]) => {
        ...
        if (view === "1rm") {
          const candidates = sessionSets.filter((s) => s.weight != null && s.reps != null).map((s) => estimate1RM(s.weight!, s.reps!));
          value = candidates.length > 0 ? (lowerIsBetter ? Math.min(...candidates) : Math.max(...candidates)) : null;
        } else if (view === "volume") {
          const total = sessionSets.filter((s) => s.weight != null && s.reps != null).reduce((acc, s) => acc + s.weight! * s.reps!, 0);
          value = total > 0 ? Math.round(total) : null;
        } else {
          const candidates = sessionSets.filter((s) => s.weight != null).map((s) => s.weight!);
          value = candidates.length > 0 ? (lowerIsBetter ? Math.min(...candidates) : Math.max(...candidates)) : null;
        }
        return { date, value };
      })
  ...
  if (points.length === 0) return <p style={{ color: "var(--dim)", fontSize: 12, textAlign: "center", padding: "20px 0" }}>No data logged yet.</p>;
  if (points.length === 1) return (
    <div style={{ textAlign: "center", padding: "12px 0" }}>
      <p style={{ color: "var(--text)", fontSize: 22, fontWeight: 600 }}>{points[0].value}</p>
      <p style={{ color: "var(--dim)", fontSize: 11 }}>{new Date(points[0].date).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</p>
    </div>
  );

  const values = points.map((p) => p.value);
  const min = Math.min(...values), max = Math.max(...values), range = max - min || 1;
  const W = 300, H = 100, PAD = 12, innerW = W - PAD * 2, innerH = H - PAD * 2;
  const coords = points.map((p, i) => ({ x: PAD + (i / (points.length - 1)) * innerW, y: PAD + (1 - (p.value - min) / range) * innerH, value: p.value, date: p.date }));
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--dim)", marginTop: 2 }}>
        ...
        <span style={{ color: trendColor, fontWeight: 600 }}>{diff > 0 ? "+" : ""}{diff}</span>
...
function HistoryPanel({ exerciseId, exercise }: { exerciseId: string | null; exercise: ExerciseRow | null }) {
  ...
  return (
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {exercise.lowerIsBetter && <p style={{ fontSize: 10, color: "var(--steel)", background: "rgba(92,122,138,0.1)", borderRadius: 4, padding: "3px 6px" }}>↓ Lower is better</p>}
      <div style={{ display: "flex", gap: 4 }}>
        {(["1rm", "volume", "heaviest"] as GraphView[]).map((v) => (
          <button key={v} onClick={() => setGraphView(v)} style={{ flex: 1, padding: "4px 0", fontSize: 10, borderRadius: 4, border: `1px solid ${graphView === v ? "var(--steel)" : "var(--line)"}`, background: graphView === v ? "var(--steel)" : "transparent", color: graphView === v ? "#fff" : "var(--dim)", cursor: "pointer" }}>
            {v === "1rm" ? "Est. 1RM" : v === "volume" ? "Volume" : "Heaviest"}
          </button>
        ))}
      </div>
      {loading ? <p style={{ color: "var(--dim)", fontSize: 12 }}>Loading…</p> : <ProgressGraph sets={sets} view={graphView} lowerIsBetter={exercise.lowerIsBetter} />}
      {sets.length > 0 && (
        <div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 14, width: 320, overflow: "hidden" }}>
        ...
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {DAY_LABELS.map((label, i) => (
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${weekdays.includes(i) ? "var(--steel)" : "var(--line)"}`,
                    background: weekdays.includes(i) ? "var(--steel)" : "transparent",
                    color: weekdays.includes(i) ? "#fff" : "var(--dim)",
                    fontWeight: weekdays.includes(i) ? 600 : 400,
                  }}
                >
                  {label}
                </button>
          ...
          <button
            onClick={handleApply}
            disabled={loading || weekdays.length === 0}
            style={{
              padding: "10px", borderRadius: 8, border: "none", cursor: "pointer",
              background: "var(--steel)", color: "#fff", fontSize: 13, fontWeight: 600,
              opacity: loading || weekdays.length === 0 ? 0.5 : 1,
            }}
          >
...
function ProgramsOverlay({ onClose, onApplied }: { onClose: () => void; onApplied: () => void }) {
  ...
  const unlocked = templates.filter((t) => t.unlocked);
  const available = templates.filter((t) => !t.unlocked);
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", inset: 0, background: "var(--bg)", zIndex: 80, display: "flex", flexDirection: "column", overflowY: "auto" }}>
        {/* Header */}
                      ...
                      {expanded && (
                        <div style={{ borderTop: "1px solid var(--line)", padding: "12px 16px" }}>
                          ...
                          {weeks.map((w) => {
                            ...
                            return (
                              <div key={w} style={{ marginBottom: 8 }}>
                                ...
                                {days.map((d) => (
                                  <p key={d.id} style={{ fontSize: 12, color: "var(--text)", paddingLeft: 8, paddingBottom: 3 }}>· {d.dayLabel}</p>
                                ))}
                              </div>
                            );
                          })}
                          <button
                            ...
                            style={{ width: "100%", marginTop: 10, padding: "10px", borderRadius: 8, border: "none", cursor: "pointer", background: "var(--steel)", color: "#fff", fontSize: 13, fontWeight: 600 }}
                          >
                            Add to my calendar
                          </button>
                        </div>
                    ...
                    <div key={t.id} style={{ border: "1px solid var(--line)", borderRadius: 10, overflow: "hidden", background: "var(--panel)", opacity: 0.7 }}>
                      ...
                      {expanded && (
                        <div style={{ borderTop: "1px solid var(--line)", padding: "12px 16px" }}>
                          ...
                          <button
                            disabled
                            style={{ width: "100%", marginTop: 10, padding: "10px", borderRadius: 8, border: "1px solid var(--line)", cursor: "not-allowed", background: "transparent", color: "var(--dim)", fontSize: 13 }}
                          >
                            Coming soon
                          </button>
                        </div>
...
function SessionPreview({ session, onSelectExercise }: { session: SessionRow | null; onSelectExercise: (ex: ExerciseRow) => void }) {
  if (!session) return (
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--line)" }}>
        ...
        {session.date && <p style={{ fontSize: 11, color: "var(--dim)", marginTop: 2 }}>{new Date(session.date).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</p>}
      </div>

      {rows.length === 0 ? (
        <div style={{ padding: 16 }}><p style={{ color: "var(--dim)", fontSize: 12 }}>No exercises in this session yet.</p></div>
      ) : (
        <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
          {rows.map((row, idx) => {
            ...
            return (
              <div key={row.id}>
                {isFirstInGroup && (
                  <div style={{ margin: "4px 12px 2px", padding: "2px 8px", borderRadius: 4, background: row.groupColor ?? "#e5e7eb", fontSize: 9, fontWeight: 600, color: "#374151", display: "inline-block" }}>
                    {row.setType === "TIME" ? "Circuit" : row.setType === "DISTANCE" ? "Interval" : "Group"}
                  </div>
                )}
                <button
                  onClick={() => onSelectExercise(row.exercise)}
                  style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "8px 16px", background: "transparent", border: "none", cursor: "pointer", textAlign: "left", borderLeft: isGrouped ? `3px solid ${row.groupColor ?? "#e5e7eb"}` : "3px solid transparent" }}
                >
                  {row.exercise.gifUrl && <img src={row.exercise.gifUrl} alt="" style={{ width: 36, height: 36, borderRadius: 4, objectFit: "cover", flexShrink: 0 }} />}
      ...
      <div style={{ padding: "10px 16px", borderTop: "1px solid var(--line)" }}>
        <a href={`/client/session/${session.id}`} style={{ display: "block", textAlign: "center", padding: "8px", background: "var(--steel)", color: "#fff", borderRadius: 6, fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
          Start workout →
        </a>
      </div>
    </div>
...
function MonthCalendar({ cursor, setCursor, sessionsByKey, selectedKey, onSelectDay }: {
  cursor: Date; setCursor: (d: Date) => void;
  sessionsByKey: Map<string, SessionRow>; selectedKey: string | null;
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 4 }}>
        {["S","M","T","W","T","F","S"].map((d, i) => <div key={i} style={{ textAlign: "center", fontSize: 9, color: "var(--dim)", padding: "2px 0" }}>{d}</div>)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 }}>
        {days.map(({ date, inMonth, key }) => {
          ...
          return (
            <div key={key} onClick={() => session && onSelectDay(key, session)} style={{ aspectRatio: "1", borderRadius: 6, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: hasSession ? "pointer" : "default", opacity: inMonth ? 1 : 0.3, background: isSelected ? "var(--steel)" : hasSession ? "rgba(92,122,138,0.15)" : "transparent", border: isToday ? "1px solid var(--steel)" : "1px solid transparent", transition: "background 0.15s" }}>
              <span style={{ fontSize: 11, color: isSelected ? "#fff" : "var(--text)", fontWeight: isToday ? 700 : 400 }}>{date.getDate()}</span>
              {hasSession && <div style={{ width: 4, height: 4, borderRadius: "50%", background: isSelected ? "#fff" : "var(--steel)", marginTop: 1 }} />}
            </div>
          );
        })}
      </div>
    ...
    <>
      ...
      {showPrograms && (
        <ProgramsOverlay
          ...
          onApplied={() => fetchSessions(monthStr)}
        />
      )}
      ...
      <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ position: "fixed", top: 14, right: 14, zIndex: 50 }}>
          ...
          {menuOpen && (
            <div style={{ position: "absolute", top: 42, right: 0, background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 10, padding: "6px 0", minWidth: 160, boxShadow: "0 8px 24px rgba(0,0,0,.4)" }}
              ...
              <button
                onClick={() => { setMenuOpen(false); setShowPrograms(true); }}
                style={{ width: "100%", padding: "10px 16px", background: "transparent", border: "none", color: "var(--text)", fontSize: 13, fontWeight: 500, textAlign: "left", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontFamily: "inherit" }}
              >
                <span>📋</span> Programs
              </button>
        ...
        <div style={{ width: 260, borderRight: "1px solid var(--line)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
            <MonthCalendar cursor={cursor} setCursor={setCursor} sessionsByKey={sessionsByKey} selectedKey={selectedKey} onSelectDay={handleSelectDay} />
            {loadingSessions && <p style={{ fontSize: 11, color: "var(--dim)", textAlign: "center", marginTop: 12 }}>Loading…</p>}
          </div>
        </div>
        ...
        <div style={{ flex: 1, borderRight: "1px solid var(--line)", display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          <SessionPreview session={selectedSession} onSelectExercise={(ex) => setSelectedExercise(ex)} />
        </div>

        {/* Right panel — Performance | History */}
        <div style={{ width: 280, display: "flex", flexDirection: "column", flexShrink: 0 }}>
          <div style={{ display: "flex", borderBottom: "1px solid var(--line)" }}>
            <a
              href="/client/performance"
              style={{ flex: 1, padding: "10px 0", fontSize: 12, background: "transparent", border: "none", borderBottom: "2px solid transparent", color: "var(--dim)", cursor: "pointer", fontWeight: 400, textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}
            >
              📈 Performance
            </a>
            <button
              onClick={() => setRightTab("templates")}
              style={{ flex: 1, padding: "10px 0", fontSize: 12, background: "transparent", border: "none", borderBottom: rightTab === "templates" ? "2px solid var(--steel)" : "2px solid transparent", color: rightTab === "templates" ? "var(--text)" : "var(--dim)", cursor: "pointer", fontWeight: rightTab === "templates" ? 600 : 400 }}
            >
              History
            </button>
          </div>
          <div style={{ flex: 1, overflowY: "auto" }}>
            {rightTab === "templates" && <HistoryPanel exerciseId={selectedExercise?.id ?? null} exercise={selectedExercise} />}
          </div>
        </div>
